...
// eta
    ...
    double c2_p[7];
     ...
     for(p_v = 2.1; p_v < 2.8; p_v+=0.1)  //  ? The Fiducial Values
     {
        ...
        uB =  (r * fp) * pow(rho_0  * pow(R_0 * M_PER_MPC,betaparameter) * pow(0.5 * (Gal[p].Qjet), 2), 1.0/3.0) * pow(2 * Gal[p].Rshocked/Hubble_h * M_PER_MPC  , (-4.0 - betaparameter)/3.0)/((r + 1.0) * (Gama_c - 1.0));
...
//          lifetime[ti] = ti;
         ...
         uB_lifetime = 0.7 * (r * fp) * pow(rho_0  * pow(R_0 * M_PER_MPC,betaparameter) * pow(0.5 * (Gal[p].Qjet), 2), 1.0/3.0) * pow(2 * Gal[p].Rshocked_lifetime[ti]/Hubble_h * M_PER_MPC  , (-4.0 - betaparameter)/3.0)/((r + 1.0) * (Gama_c - 1.0));
